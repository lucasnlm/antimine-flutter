...
abstract class MinefieldCreator {
  /// Creates a minefield.
  Future<List<Area>> create(Minefield minefield, int x, int y, bool useForms);

  /// Creates a empty minefield.
  Future<List<Area>> createEmpty(Minefield minefield, bool useForms) async {
    return List.generate(minefield.height, (y) {
  ...
  /// Creates a empty minefield.
  List<Area> fromList(Minefield minefield, bool useForms, List<Area> areas) {
  ...
  Neighbours getNeighbours(int x, int y, int width, int height) {
    return Neighbours(
  ...
  List<int> calcNeighbours(int x, int y, int width, int height) {
    return <Vector2>[
          Vector2(x - 1, y - 1),
          Vector2(x - 1, y + 0),
          Vector2(x - 1, y + 1),
