...
void main() {
  ...
  test('create empty', () async {
    ...
    // Then
    expect(handler.result().length, equals(100));
    expect(handler.result().any((e) => e.hasMine), isFalse);
  });
  ...
  test('test consume', () {
    ...
    handler.consume(
      GameState(
        areas: List.generate(10, (index) => Area(id: index, x: 0, y: 0)),
        minefield: minefield,
    ...
    // Then
    expect(handler.result().length, equals(10));
  });
  ...
  test('test plantMines', () async {
    ...
    // Then
    expect(handler.result().length, equals(100));
    expect(handler.result().where(hasMine).length, equals(9));
  });
  ...
  test('test hasMines true', () {
    ...
    handler.consume(
      GameState(
        ...
        areas: List.generate(
          10,
          (index) => Area(id: index, x: 0, y: 0, hasMine: index == 5),
        ),
        minefield: minefield,
        params: const GameParams(),
      ),
  ...
  test('test openArea', () async {
    ...
    // When
    await handler.plantMinesFor(Vector2(3, 3), true, true);
    expect(handler.result()[55].covered, isTrue);

    handler.openByPosition(Vector2(5, 5), openNeighbors: false);

    // Then
    expect(handler.result()[55].covered, isFalse);
  });
  ...
  test('test openArea and neighbors', () async {
    ...
    // When
    await handler.plantMinesFor(Vector2(3, 3), true, true);
    expect(handler.result().where(isCovered).length, 100);

    handler.openByPosition(Vector2(3, 3));

    // Then
    expect(handler.result().where(isCovered).length, 31);
  });
  ...
  test('openById and neighbors', () async {
    ...
    // Then
    expect(handler.result().where(isCovered).length, 22);
  });

  test('test forms update', () async {
    // Given
    await handler.createEmpty(minefield, true);
    expect(handler.result().where(hasNoForm).length, 0);

    // When
    await handler.plantMinesFor(Vector2(3, 3), true, true);
    expect(handler.result().where(hasForm).length, 100);
