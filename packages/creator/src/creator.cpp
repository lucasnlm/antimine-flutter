    ...
    int mask)
{
  ...
  std::stable_sort(square_list.begin(), square_list.end(), compare_square);
...
const char *createMinefield(
    ...
    intptr_t inY)
{
  static std::string minefield;
  std::mt19937 random(inSeed);
  minefield = new_mine_layout(sliceWidth, inWidth, inHeight, inMines, inX, inY, random);
  return minefield.c_str();
}
